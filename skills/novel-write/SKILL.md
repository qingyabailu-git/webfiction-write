...
| 位置 | 提示内容 |
|------|---------|
| 第 1 章 | "第 1 章已完成。建议立即做一次深度审查：novel-review 1（deep 模式）。这章是全书的脸面。" |
| 第 5 章 | "连续写了 5 章。建议：novel-review 1-5 批量审查；检查追踪/伏笔.md；回看第 1 章风格一致性。" |
| 第 10 章 | "第 10 章完成。建议完整 review-pipeline：novel-review 1-10；运行一致性检查；查看 dashboard 伏笔密度。" |
| 卷最后一章 | "「{卷名}」全部写完。建议：卷末深度审查；卷摘要归档；规划下一卷：novel-plan {下一卷号}" |
| 距上次审查 ≥5 章 | "距上次审查过了 5 章。建议：novel-review {上次+1}-{当前}" |
...
- 修改已写章节 → novel-revise
- 单独审查 → novel-review
- 单独润色/去 AI 味 → novel-polish
- 批量更新细纲 → novel-plan
- 查询章节/设定状态 → novel-query
- 可视化面板 → novel-dashboard
- 项目体检 → novel-doctor
- 提取写作模式 → novel-learn
- 多书切换 → novel-switch
- 导出/完本 → novel-export
