...
description: |
  ...
  触发方式：/novel-polish {章号或范围}、「润色」「去AI味」「格式」「标点规范」。
---

# novel-polish：润色与去 AI 味

独立于 novel-write 的 Step 4，专门用于用户想单独润色场景。
...
```bash
novel-polish 1-10        # 润色第 1-10 章
novel-polish --all        # 全部已写章节
novel-polish --volume 1  # 整卷
```
...
文风一致性维度由 novel-conceive 阶段产出的文风档案.md 定义。
本 skill 不内联上述参考文件内容，运行时按需加载。
转换脚本 normalize-punctuation.js 可从 story-* 系列脚本目录复制使用。

## 与 novel-write 的关系

novel-write 的 Step 4 内嵌了相同的润色流程。
novel-polish 是独立的外置版本，用于单独调用。
两者执行效果一致，不应出现同一内容两个不同结果。
如果 novel-write 时已执行过 Step 4，再次跑 novel-polish 不会重复修改。
（但如果用户手动改过正文，novel-polish 会基于最新版本再次处理。）
