...
reviewer 只返回严格结构化 JSON，不评分，不口头总结。主线负责把返回的 JSON 写入 `.novel/tmp/review_results.json`。
...
- review_metrics 写入 metrics 存储（review-pipeline --save-metrics）
