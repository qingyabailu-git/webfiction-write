...
导出不是终点，是读者看到的样子。导出时帮用户考虑：

1. **导出前要不要先润色一遍？** — 提醒用户跑一下 fiction-polish 或 fiction-doctor 检查
2. **要导出什么格式最合适？** — 番茄平台投 txt，自留存 .docx，看场景给建议
3. **导出后别急着关项目** — 完本归档后项目仍然可查可改，需要修改随时回来
...
| 相关技能 | 关系 | 使用场景 |
|---------|------|---------|
| **fiction-write** | 写→导出 | 写完章节后累积，随时可导出汇总 |
| **fiction-doctor** | 导出前检查 | 导出前建议先跑项目体检，确保无缺失章节 |
