...
name: fiction-revise
description: |
  章节修改。支持三种模式：Polish（润色表达，不改事实）、大改（改事实但主干不变，
  自动传播到后续章节和设定）、重写（整章重写，全链路刷新底本和数据层）。
  系统自动检测修改类型并推荐模式。
...
| **大改** | 改了事实（人名/情节/细节），主干不变 | 自动传播 | 20-40 分钟 |
| **重写** | 整章推翻重来，情节走向变了 | 全链路刷新 | 60-90 分钟 |
...
| 用户说"把 XXX 改成 YYY""设定不对" | 大改 |
| 用户说"这章重写""推倒重来""重来一遍" | 重写 |
| 修改范围超过一章（"第 5-8 章都要改"） | 重写 |
| 修改波及设定集（"世界观改了""主角能力改"） | 重写（广播） |
