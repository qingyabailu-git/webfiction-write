...
name: fiction-write
description: |
  正文写作。支持 普通模式（标准四步）和 深度模式（完整六步+底本同步+data-agent）双模式。
...
| 模式 | 适用场景 | 耗时 |
|------|---------|------|
| **普通模式** | 常规推进章 | ~30-40 分钟/章 |
| **深度模式** | 第 1 章/卷首/卷末/高潮章/里程碑章 | ~60-90 分钟/章 |
...
| 本章是某卷的第 1 章 | 深度模式 |
| 本章是某卷的最后 2 章 | 深度模式 |
| 本章是第 10/20/30... 章（里程碑） | 深度模式 |
| 章纲标记了 `climax: true` / `is_reversal: true` | 深度模式 |
| 章纲标记了 `mode: deep` | 深度模式 |
| 以上都不是的常规章 | 普通模式 |
...
- 设定集/文风档案.md → 深度模式 模式下检查文风一致性并追加记录
...
<!-- fiction-write: ����д�� 普通模式/深度模式 ˫ģʽ�Զ��ж� -->
