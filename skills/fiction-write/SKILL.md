...
name: fiction-write
description: |
  正文写作 + 章节修改。写作支持 Normal（标准四步）和 Deep（完整六步+底本同步+data-agent）双模式；修改支持 Re-craft（大改，自动传播）和 Rewrite（重写，全链路刷新）两种内建模式。Polish 模式请走 fiction-polish。
...
本 skill 同时承担章节修改职责（原 fiction-revise 已合并进来）。一次修订 = 定位变更点 → 执行修订 → 探测影响范围 → 传播变更 → 校验一致性。
...
| **Re-craft（大改）** | 改了事实（人名/情节/细节），主干不变 | 自动传播 | 20-40 分钟 | 本 skill 内建 |
| **Rewrite（重写）** | 整章推翻重来，情节走向变了 | 全链路刷新 | 60-90 分钟 | 本 skill 内建 |
