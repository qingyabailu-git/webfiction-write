...
- 存在则提示"该项目已初始化，可直接 novel-start 或 novel-conceive"
- 不存在则继续
- 询问用户书名，生成安全化目录名
...
├─ .novel/
│   ├─ state.json            # 由后续步骤填充
│   ├─ idea_bank.json        # 由 novel-conceive 填充
│   └─ tmp/                  # 运行时临时文件
│
├─ 设定集/                    # 世界观、角色、力量体系等
├─ 大纲/                      # 总纲、卷纲、章纲
...
- 列出创建的文件和目录
- 建议下一步：novel-conceive（构思）或 novel-start（如果已有想法）

## 参考

详细目录结构约定见 `references/`（暂无，按上述硬编码）。
nove-cover 在需要时单独调用，不在此处生成。
扫榜工具 novel-scan 需要浏览器操作，详见该 skill。
拆文工具 novel-analyze 需要提供小说文本，详见该 skill。
