...
> **何时加载**：fiction-analyze Stage 6 执行时。前置依赖：Stage 0-5 已完成，`拆文报告.md` + `章节/*_摘要.md` + `章节/第1-3章_深度拆解.md` + `原文/原文.txt`（或 `.md`）齐全。
>
...
- **文风文件必须留在拆文库**（`拆文库/{书名}/文风.md`），**永不写入** `对标/` 或写作项目目录——拆文库是 analyze 的数据源，写作项目的 `对标/{书名}/` 由 fiction-import 从拆文库同步
...
- fiction-import 把整个 `拆文库/{书名}/` 同步到项目 `{项目}/对标/{书名}/` 时**自动包含**文风（与拆文报告同等待遇）
- 写作端（fiction-write）的日更循环读 `{项目}/对标/{书名}/文风.md`（按对标书路径查找规则，回退 `拆文库/{书名}/`）

## 重生策略（旧拆文库无文风）

旧 `拆文库/{书名}/` 没有文风文件时：

- **完整重跑** `/fiction-analyze`：开销大，会重跑 Stage 0-5（不必要）
