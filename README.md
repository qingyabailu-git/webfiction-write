...
> 我不懂编程，这个 skill 是用 Codex 接入 深度模式Seek 写的。
# web-fiction - 网文创作工具集
...
| **fiction-write** | 写正文（系统自动判断用普通写还是深度写） | 「写第X章」「续写」「接着写」「下一章」「日更」 | 核心 |
| **fiction-revise** | 修改章节（小改文字/大改剧情/全部重写） | 「修改第X章」「重写」「这章不太对」「不满意」「回炉」 | 写作后 |
...
write    → 开始写作（系统自动推荐模式、提醒你该做什么）
     ↓
revise   → 修改（改了一处，系统会自动检查后续章节有没有受影响）
     ↓
...
| 修改方式 | 改剧情？ | 自动检查后续影响？ | 适用场景 |
|------|---------|-----------|------|
| 小修（润色） | ❌ | ❌ | 只调文字、润色 |
| 大改（大改） | ✅ | ✅ | 改剧情，系统自动看后面有没有矛盾 |
| 重写（重写） | ✅ 全部 | ✅ | 推翻重来，全链路刷新 |
